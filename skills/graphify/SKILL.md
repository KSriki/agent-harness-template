...
name: graphify
description: >
  Use for STRUCTURE and relationship questions about a codebase/corpus — how X
  reaches Y, what depends on what, blast radius, architectural hubs — and to build
  or update the knowledge graph behind them ("/graphify", "query the code graph",
  "map this repo"). When graphify-out/ already exists, prefer a graphify query for
  such questions. Turns any input (code, docs, papers, images, videos) into a
  persistent knowledge graph with god nodes, community detection, and
  query/path/explain tools.
  Do NOT use to locate/read a specific symbol or file (`code-searcher` / Explore —
  grep beats a graph for "where is X defined") or for questions outside the repo
  (`debug-research`).
---

<!--
  PROVENANCE — vendor skill from `graphify install` (graphifyy v0.9.27, Apache-2.0/MIT),
  security-reviewed 2026-07-28 and adopted with two harness edits:
    1. description bounded + NOT-clause (was: "use for ANY question... query first")
    2. Step 1 auto-install/upgrade replaced with a propose-to-human stop (guardrail #3)
  DO NOT re-run `graphify install` — it would clobber these reviewed edits. Update
  path: fetch the new vendor version, diff, re-review, re-apply (evolve-harness).
-->
...
if ! "$PYTHON" -c "import graphify" 2>/dev/null; then
    # HARNESS EDIT (guardrail #3 — no silent installs or upgrades, ever):
    # the vendor version auto-ran `uv tool install --upgrade` here. Instead: STOP,
    # propose the install to the human, and continue only after they run it.
    echo "graphify is not installed on this machine." >&2
    echo "Ask the human to run:  uv tool install graphifyy   — then retry this step." >&2
    exit 1
fi
